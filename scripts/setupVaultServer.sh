...
GCP_CREDENTIALS=$(awk -F= -v key="gcp_credentials_file" '$1==key {print $2}' /vagrant/data.txt)
GCP_PROJECT=$(awk -F= -v key="gcp_project" '$1==key {print $2}' /vagrant/data.txt)
GCP_REGION=$(awk -F= -v key="gcp_region" '$1==key {print $2}' /vagrant/data.txt)
GCP_KEYRING=$(awk -F= -v key="gcp_keyring" '$1==key {print $2}' /vagrant/data.txt)
GCP_CRYPTO_KEY=$(awk -F= -v key="gcp_crypto_key" '$1==key {print $2}' /vagrant/data.txt)
